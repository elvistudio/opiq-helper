...
`materials_resolved: true` is rejected if a declared material or key is absent. `print_ready: true` is rejected if a required student file is not printable. `schemas/teacher-review.schema.json` and `schemas/classroom-trial.schema.json` define the independent evidence records. Approved review requires a registered, completed record with role, date, mandatory scope, closed blocking/major findings, closed required changes, and the current teacher-pack content commit SHA. Tested status requires a registered, analysed trial with at least one lesson, complete privacy declarations, a successful decision, no open safety blocker, and the same current pack SHA.

The teacher-pack content SHA is frozen with the command documented in `pedagogical-reviews/grade-5-science/water/review-guide.md`. `materials-index.yaml` is excluded from that content history because it carries the evidence link; registering a record must not invalidate the record by itself. Any later change to teacher/student/parent pack content makes linked evidence stale. Git timestamps are not evidence.
...
The evidence workflow is: (1) merge the authored pack, (2) freeze the reviewed SHA, (3) conduct independent teacher review, (4) record findings, (5) resolve required changes, (6) conduct a limited classroom trial, (7) analyse anonymised observations, and (8) update readiness in a separate PR. `classroom_ready: true` requires both current evidence records and is rejected while blocking, safety, required-change, or stale-evidence diagnostics remain.
