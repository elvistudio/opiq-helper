...
The next readiness phase is evidence-based: merge the authored pack, freeze its content commit SHA, complete an independent review, record and resolve findings, run a limited classroom trial, analyse only anonymised aggregate observations, then update readiness in a separate PR. Blank templates do not count as evidence, and review/trial records for an older pack commit are stale. The process does not store student personal data; automated declarations supplement but do not replace human privacy review.
...
`npm run check:reviews` validates the blank production templates, registered evidence records, mandatory review scope, anonymised trial declarations, current commit-SHA binding, stale evidence, finding resolution, and the final classroom-readiness gate. Current production has 0 completed reviews and 0 analysed trials and therefore passes with two honest workflow warnings.
