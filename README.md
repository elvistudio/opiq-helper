...
The generator removes repeated kit-detail covers and administrative pages, requires unique canonical chapter URLs, assigns source-book-plus-kit identities where one source ID spans several kits, and labels simplified-curriculum records. Grade 2 first-language Estonian and Estonian as a second language are disjoint routes generated truthfully from one shared original archive; see the [Estonian subject-separation audit](docs/audits/grade-2-estonian-subject-separation.md). The science route may register additional book-level archives without hiding provenance: the dedicated kit 330 capture contributes 27 `Minu väike kallis planeet 2 klass` pages and removes their incorrect legacy placement in grade 1; see the [kit 330 routing audit](docs/audits/grade-2-minu-vaike-kallis-planeet.md). The combined science/human-studies export is deliberately not routed as one subject: the canonical `grade-2-science` index excludes the two mixed books, while `grade-2-human-studies` retains its declared archive boundary. Book corrections and the no-recapture follow-up are recorded in the [catalogue metadata audit](docs/audits/grade-2-catalog-metadata.md). None of these routes is a verified curriculum map.
